IF EXISTS (SELECT 1 FROM SYS.PROCEDURES WHERE NAME ='SP_ALTER_COLALTE')
    ...
    END 
GO
CREATE PROCEDURE SP_ALTER_COLALTE (@Collation VARCHAR (30) =NULL) AS 
 IF @Collation IS NULL 
    BEGIN
    SELECT 'POR FAVOR, INSIRA A COLLATE' 
    RETURN
    END 
...
DECLARE @sCmd VARCHAR(600), @Tbl VARCHAR(1000), @cln VARCHAR(1000),@NOME_INDEX VARCHAR (1000),@DROP_INDEX VARCHAR(MAX),@CREATE_INDEX VARCHAR(MAX);
PRINT 'LOOP PARA PEGAR AS TABELAS'
...
FOR 
    SELECT c.TABLE_NAME,
           'ALTER TABLE ['+c.TABLE_NAME+'] ALTER COLUMN ['+COLUMN_NAME+'] '+CASE
                                                                            ...
                                                                            END+' COLLATE SQL_Latin1_General_CP1_CI_AS '+CASE IS_NULLABLE
                                                                                                                             WHEN 'YES'
                                                                                                                             THEN 'NULL'
                                                                                                                             ELSE 'NOT NULL'
                                                                                                                         END,
...
PRINT 'CURSOR PARA PEGAR OS INDICES '
 DECLARE @ID_COL INT 
 declare BUSCA_INDICES cursor  FAST_FORWARD FOR 
 
	   SELECT DISTINCT (NOME_INDEX) FROM #INDICES
	...
	BEGIN
	  ...
	  -- E PRIMARY KEY   
	   ...
	   END
	   ELSE  IF EXISTS (SELECT 1 FROM #indices WHERE IS_UNIQUE = 1 AND IS_PRIMARY_KEY = 0 and NOME_INDEX=@NOME_INDEX)	
	   BEGIN
	      -- E UNIQUE KEY
		  SELECT
		  @DROP_INDEX ='DROP INDEX '+ @NOME_INDEX+ ' ON '+TABELA,
		  @CREATE_INDEX = 'CREATE UNIQUE NONCLUSTERED INDEX '+@NOME_INDEX+ ' ON ' +TABELA +'('
		  FROM #INDICES
			WHERE NOME_INDEX= @NOME_INDEX

		  PRINT 'CURSOR PARA PEGAR OS UNIQUES '
...
FETCH NEXT FROM MONTA_INDICE_UNIQUE INTO @ID_COL;
	   ...
	   END
	   ELSE IF EXISTS (SELECT 1 FROM #indices WHERE IS_UNIQUE = 0 and  NOME_INDEX=@NOME_INDEX)
		  	   BEGIN
		  ...
		  FROM #INDICES
			WHERE NOME_INDEX= @NOME_INDEX
			PRINT 'CURSOR PARA PEGAR OS NONCLUESTERS '
...
FROM #COMANDOS  com inner join 
     #tmpErro err on 
	tabela=nome_tabela
--WHERE NOME_INDEX ='IX_PDV_CONSLD_ERROS_DT_MOV'
